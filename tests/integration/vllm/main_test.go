...
// Package vllm_integration exercises deploy/compose/vllm.yml. Env-gated:
// the test skips unless LLM_INIT_INTEGRATION_VLLM=1 is set, since vLLM
// requires NVIDIA GPUs and CI runners do not provide them.
//
// Local invocation (chat, default):
//
//	LLM_INIT_INTEGRATION_VLLM=1 go test -tags integration -timeout 30m \
//	    ./tests/integration/vllm/...
//
// Local invocation (embedding; see llminit/integration_test_vllm_embedding.sh):
//
//	LLM_INIT_INTEGRATION_VLLM=1 MODEL_MODE=embedding MODEL_SOURCE='hf://...' \
//	    go test -tags integration -timeout 30m ./tests/integration/vllm/...
package vllm_integration
...
// running embedding (BAAI/bge-small-en-v1.5 + --runner pooling).
...
func TestVLLM_FullStack(t *testing.T) {
	if os.Getenv("LLM_INIT_INTEGRATION_VLLM") == "" {
		t.Skip("set LLM_INIT_INTEGRATION_VLLM=1 (and provide an NVIDIA GPU) to enable")
	}
	...
	composePath := repoRelative(t, "deploy/compose/vllm.yml")
	stack := shared.Up(t, composePath)

	// vLLM cold start with weight download + CUDA graph capture often
	// takes 5-10 min on a fresh runner; allow generous slack.
	...
	shared.RecordBaseline(t, "vllm", ready, dataLatency)
}
